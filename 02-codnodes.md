...
Let see how busy the node is. Write this command:
...
The first percentage represents the percentage the CPUs are busy for the different user processes. An explanation of the other numbers can be found [here](http://www.unixtutorial.org/commands/top/) (look for 'CPU(s) status')
...
These lines describe memory usage and swap usage. Swap memory is define [here](https://www.centos.org/docs/5/html/5.2/Deployment_Guide/s1-swap-what-is.html):
...
* VIRT, RES, SHR – deal with memory usage. VIRT stands for *virtual* size, and 'represents how much memory the program is able to access at the present moment'. RES stands for the *resident part* of a process – how much of it resides in the physical memory and this number is more relevant for the actual memory usage of processes. SHR can be ignore. For more details, see for example [this explanation](http://mugurel.sumanariu.ro/linux/the-difference-among-virt-res-and-shr-in-top-output/).
...
* %CPU – percentage of the time shares CPU spends running a particular process. Note that 100% represents one CPU, so the `blastx` process in the example above uses the equivalent of almost 6 CPUs.
* %MEM – percentage of the physical memory of your system which is used by each process (the number from the `RES` column as a fraction of the total memory
...
* `hannibal` runs `blastx` with almost 6 CPUs and very little memory
* `superman` runs `samtoh5` with one CPU and 48 GB of memory
* `username` just has started running the `top` command

###`top` tips

After typing `top`, you an then use some keys to change what is displayed:
...
>
> use `top -u username` to start top with only showing processes belonging to you. Press `u` and enter to show processes for all users again.

> ## Exercise {.challenge}
>
> Run top and check the different users that are on the system. Who do you think user `root` is?

One good reference for the `top` command is this webpage: <http://www.computerhope.com/unix/top.htm>
...
```
This script will sleep (unix `sleep` command) a user-defined amount of times (variable `rounds`) for a user-defined amount of seconds (variable `seconds`).
...
> ## Exercise {.challenge}
>
> Is the `sleep` command the only one appearing in the `top` output when you start the script?

###Using `screen` for long running jobs

If you are starting a long running job you cannot close the terminal window because the job will be cancelled. A solution to this problem is using the so-called unix `screen`. This starts a virtual terminal that can be kept 'alive' while disconnection from the server.
...
You can start multiple hidden windows by just repeating the `screen` command. If you have multiple hidden windows running the `screen -rd` command will give you a list of the windows available. To choose one of the windows just add the window-number after the command, for example:

~~~ {.bash}
screen -rd 45142.pts-15.cod5
~~~

When your job is finished you need to close the hidden window by typing `exit`, or `ctrl-d`, instead of `ctrl-a-d`

> ## Callout Box {.callout}
>
> you can also name screens using `screen -S sometime` and reattach using the same name: `screen -rd sometime`. `screen -list` shows all your active screen on the node
...
Using `top` will show you the processes that are running, but not necessarily much detail on what they are doing. Many programs will generate output to the screen or write to log files. In this part, you'll learn how to inspect the log files, including how to see new output being added.
...
Note how it is safe to cancel the `tail -f` command using `crtl-c`, as this does not stop the actual process (which rund in a `screen`). In fact, if you let the `rounds.sh` script finish, the `tail -f` command still will wait for new input, which in this case never comes. So, you'll have to cancel the `tail -f` to get back to being able to use the terminal.
